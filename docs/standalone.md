...
Setting `attach_fn` is what makes this possible: the connect is only deferred when the library attaches the adapter itself, which is when the asynchronous resolution can be started.
...
This means the legacy pattern resolves synchronously even with a c-ares aware adapter, since resolution has already completed by the time the adapter is attached.
