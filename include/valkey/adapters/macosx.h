...
static void valkeyMacOSAsyncCallback(CFSocketRef __unused s,
...
static int valkeyMacOSAttach(valkeyAsyncContext *valkeyAsyncCtx,
    ...
    ValkeyRunLoop *valkeyRunLoop =
        (ValkeyRunLoop *)vk_calloc(1, sizeof(ValkeyRunLoop));
    if (valkeyRunLoop == NULL)
        return VALKEY_ERR;
    ...
    valkeyRunLoop->socketRef = CFSocketCreateWithNative(
        NULL, valkeyCtx->fd, kCFSocketReadCallBack | kCFSocketWriteCallBack,
        valkeyMacOSAsyncCallback, &socketCtx);
    if (!valkeyRunLoop->socketRef)
        return freeValkeyRunLoop(valkeyRunLoop);

    valkeyRunLoop->sourceRef =
        CFSocketCreateRunLoopSource(NULL, valkeyRunLoop->socketRef, 0);
    if (!valkeyRunLoop->sourceRef)
        return freeValkeyRunLoop(valkeyRunLoop);

    CFRunLoopAddSource(runLoop, valkeyRunLoop->sourceRef,
