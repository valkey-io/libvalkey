...
#define valkeyTestPanic(msg)                                                   \
    do {                                                                       \
        fprintf(stderr,                                                        \
                "PANIC: %s (In function \"%s\", file \"%s\", line %d)\n", msg, \
...
static void test_format_commands(void) {
    ...
    test("Format command with an empty string in between proper "
         "interpolations: ");
    len = valkeyFormatCommand(&cmd, "SET %s %s", "", "foo");
    test_cond(strncmp(cmd, "*3\r\n$3\r\nSET\r\n$0\r\n\r\n$3\r\nfoo\r\n", len) ==
                  0 &&
    ...
    len = valkeyFormatCommand(&cmd, "SET %b %b", "foo", (size_t)3, "b\0r",
                              (size_t)3);
    test_cond(strncmp(cmd, "*3\r\n$3\r\nSET\r\n$3\r\nfoo\r\n$3\r\nb\0r\r\n",
                      len) == 0 &&
              len == 4 + 4 + (3 + 2) + 4 + (3 + 2) + 4 + (3 + 2));
    ...
    len =
        valkeyFormatCommand(&cmd, "SET %b %b", "foo", (size_t)3, "", (size_t)0);
    test_cond(strncmp(cmd, "*3\r\n$3\r\nSET\r\n$3\r\nfoo\r\n$0\r\n\r\n", len) ==
                  0 &&
              len == 4 + 4 + (3 + 2) + 4 + (3 + 2) + 4 + (0 + 2));
    vk_free(cmd);
...
#define INTEGER_WIDTH_TEST(fmt, type)                                          \
    do {                                                                       \
        ...
        len = valkeyFormatCommand(&cmd, "key:%08" fmt " str:%s", value,        \
                                  "hello");                                    \
        test_cond(strncmp(cmd,                                                 \
                          "*2\r\n$12\r\nkey:00000123\r\n$9\r\nstr:hello\r\n",  \
                          len) == 0 &&                                         \
                  len == 4 + 5 + (12 + 2) + 4 + (9 + 2));                      \
        vk_free(cmd);                                                          \
...
#define FLOAT_WIDTH_TEST(type)                                                 \
    ...
    test("Format command with unhandled printf format (specifier 'p' not "
         "supported): ");
    len = valkeyFormatCommand(&cmd, "key:%08p %b", (void *)1234, "foo",
                              (size_t)3);
    test_cond(len == -1);
...
static void test_tcp_options(struct config cfg) {
    ...
    test_cond(valkeySetTcpUserTimeout(c, 100) == VALKEY_ERR &&
              c->err == VALKEY_ERR_IO);
#endif
...
static void test_unix_keepalive(struct config cfg) {
    ...
    test("Setting TCP_KEEPALIVE on a unix socket doesn't break the "
         "connection: ");
...
static void test_reply_reader(void) {
    ...
    ret = valkeyReaderGetReply(reader, &reply);
    test_cond(ret == VALKEY_ERR &&
              strcasecmp(reader->errstr, "Bulk string length out of range") ==
                  0);
    freeReplyObject(reply);
    valkeyReaderFree(reader);
    ...
    ret = valkeyReaderGetReply(reader, &reply);
    test_cond(
        ret == VALKEY_OK && ((valkeyReply *)reply)->type == VALKEY_REPLY_PUSH &&
    ...
    test_cond(ret == VALKEY_OK &&
              ...
              fabs(((valkeyReply *)reply)->dval - 3.14159265358979323846) <
                  0.00000001 &&
              ((valkeyReply *)reply)->len == 22 &&
              strcmp(((valkeyReply *)reply)->str, "3.14159265358979323846") ==
                  0);
    freeReplyObject(reply);
    valkeyReaderFree(reader);
    ...
    ret = valkeyReaderGetReply(reader, &reply);
    test_cond(
        ret == VALKEY_OK && ((valkeyReply *)reply)->type == VALKEY_REPLY_MAP &&
    ...
    ret = valkeyReaderGetReply(reader, &reply);
    test_cond(
        ret == VALKEY_OK && ((valkeyReply *)reply)->type == VALKEY_REPLY_ATTR &&
        ((valkeyReply *)reply)->elements == 4 &&
        ((valkeyReply *)reply)->element[0]->type == VALKEY_REPLY_STATUS &&
    ...
    ret = valkeyReaderGetReply(reader, &reply);
    test_cond(
        ret == VALKEY_OK && ((valkeyReply *)reply)->type == VALKEY_REPLY_SET &&
    ...
    test_cond(ret == VALKEY_OK &&
              ...
              fabs(((valkeyReply *)reply)->element[0]->dval -
                   3.14159265358979323846) < 0.00000001 &&
              ((valkeyReply *)reply)->element[0]->len == 22 &&
              strcmp(((valkeyReply *)reply)->element[0]->str,
...
static void test_resp3_push_options(struct config config) {
    ...
    test("We can use valkeyOptions to set a custom PUSH handler for "
         "valkeyContext: ");
    options = get_server_tcp_options(config);
...
static void test_blocking_connection(struct config config) {
    ...
    reply = valkeyCommand(c, "SET %b %b", "foo", (size_t)3, "hello\x00world",
                          (size_t)11);
    freeReplyObject(reply);
    reply = valkeyCommand(c, "GET foo");
    test_cond(reply->type == VALKEY_REPLY_STRING &&
    ...
    test_cond(reply->type == VALKEY_REPLY_NIL) freeReplyObject(reply);
    ...
    test_cond(reply->type == VALKEY_REPLY_ARRAY && reply->elements == 2 &&
...
static int detect_debug_sleep(valkeyContext *c) {
    ...
    if (reply == NULL || c->err) {
        ...
        fprintf(stderr,
                "Error testing for DEBUG SLEEP (server error: %s), exiting\n",
                cause);
        exit(-1);
    }
...
static void test_blocking_connection_timeouts(struct config config) {
    ...
    reply = valkeyCommand(c, "GET foo");
    test_cond(reply != NULL && reply->type == VALKEY_REPLY_STRING &&
...
#ifndef _WIN32
    test_cond(respcode == VALKEY_ERR && c->err == VALKEY_ERR_IO &&
              errno == EAGAIN);
#else
...
static void test_invalid_timeout_errors(struct config config) {
    ...
    test("Set error when an invalid timeout usec value is used during "
         "connect: ");
    ...
    if (config.type == CONN_TCP || config.type == CONN_SSL) {
        c = valkeyConnectWithTimeout(config.tcp.host, config.tcp.port,
                                     config.connect_timeout);
    } else if (config.type == CONN_UNIX) {
        c = valkeyConnectUnixWithTimeout(config.unix_sock.path,
                                         config.connect_timeout);
    } else {
        valkeyTestPanic("Unknown connection type!");
    }

    test_cond(c != NULL && c->err == VALKEY_ERR_IO &&
...
void subscribe_cb(valkeyAsyncContext *ac, void *r, void *privdata) {
    ...
    assert(reply != NULL &&
           reply->type ==
               (state->resp3 ? VALKEY_REPLY_PUSH : VALKEY_REPLY_ARRAY) &&
           reply->elements == 3);
...
void subscribe_with_timeout_cb(valkeyAsyncContext *ac, void *r,
                               void *privdata) {
    valkeyReply *reply = r;
    TestState *state = privdata;
...
static void test_command_timeout_during_pubsub(struct config config) {
    ...
    /* Start subscribe */
    TestState state = {.options = &options, .resp3 = 1};
    valkeyAsyncCommand(ac, subscribe_with_timeout_cb, &state,
                       "subscribe mychannel");
...
static void commandCallback(struct valkeyAsyncContext *ac, void *_reply,
                            void *_privdata);
...
int main(int argc, char **argv) {
    struct config cfg = {.tcp = {.host = "127.0.0.1", .port = 6379},
                         .unix_sock = {.path = "/tmp/valkey.sock"}};
    int throughput = 1;
    ...
    if (cfg.ssl.port && cfg.ssl.host) {
        ...
        valkeyInitOpenSSL();
        _ssl_ctx = valkeyCreateSSLContext(cfg.ssl.ca_cert, NULL, cfg.ssl.cert,
