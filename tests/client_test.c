...
static void consume_timestamp_error(valkeyFD fd) {
    union {
...
static void test_connect_error_queue(int blocking) {
    ...
    context = blocking ? valkeyConnect("127.0.0.1", ntohs(address.sin_port)) : valkeyConnectNonBlock("127.0.0.1", ntohs(address.sin_port));
    ...
    if (setsockopt(context->fd, SOL_SOCKET, SO_TIMESTAMPING,
                   &timestamping, sizeof(timestamping)) == -1) {
        test(blocking ? "Blocking connect error queue: " : "Nonblocking connect error queue: ");
        printf("SO_TIMESTAMPING unavailable (%s): ", strerror(errno));
        test_skipped();
        goto cleanup;
    }
    ...
    assert(valkeyCheckConnectDone(context, &completed) == VALKEY_OK);
    assert(completed == 1);
    count = epoll_wait(epoll_fd, &event, 1, 0);
    test(blocking ? "Successful blocking connect drains the error queue: " : "Successful nonblocking connect drains the error queue: ");
    test_cond(count == 0);
    ...
    assert(valkeyBufferRead(context) == VALKEY_OK);
    assert(valkeyGetReplyFromReader(context, (void **)&reply) == VALKEY_OK);
...
int main(int argc, char **argv) {
    ...
    test_free_null();
#ifdef __linux__
    test_connect_error_queue(0);
    test_connect_error_queue(1);
#endif
